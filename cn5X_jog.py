...
class dlgJog(QObject):
  ...
  def __init__(self, grbl: grblCom, decoder, axisNumber: int, axisNames: list):
    ...
    if self.__nbAxis > 3:
      self.di.chkJogU.setText(self.tr("Jog {} axis".format(self.__axisNames[3])))
      self.di.btnJogU.changeIcon(":/cn5X/images/btnJog{}.svg".format(self.__axisNames[3]))
      self.di.chkJogU.setEnabled(True)
      self.di.dsbJogU.setEnabled(True)
    else:
      self.di.chkJogU.setText(self.tr("Jog - axis"))
      self.di.btnJogU.changeIcon(":/cn5X/images/btnJogNone.svg")
      self.di.chkJogU.setEnabled(False)
      self.di.dsbJogU.setEnabled(False)
    if self.__nbAxis > 4:
    ...
    self.di.btnJogU.clicked.connect(lambda: self.on_btnJog(3, self.di.dsbJogU))
    ...
    self.di.chkJogU.toggled.connect(lambda: self.on_chkJog_toogle('U'))
    ...
    self.di.chkJogU.stateChanged.connect(lambda: self.on_chkJog_changed(self.di.chkJogU, 3))
    ...
    self.di.dsbJogU.valueChanged.connect(lambda: self.on_dsbJog_changed(self.di.dsbJogU, self.di.chkJogU, 3))
